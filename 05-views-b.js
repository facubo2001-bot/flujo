...
function viewCartera() {
  ...
  let html = '';
  html += `<div class="card tight" style="margin-bottom:14px"><div class="row between" style="flex-wrap:wrap;row-gap:12px"><div style="min-width:0;flex:1 1 230px"><b style="font-weight:800">${k.conPrecio ? `Precios al ${fechaTxt}` : 'Sin precios todavía'}</b><span class="sub">${hayKey ? `MEP $ ${fmtARS.format(k.mep)}${s.ccl ? ' · CCL $ ' + fmtARS.format(k.ccl) : ''}${k.spyHoy ? ' · SPY ' + fmtU(k.spyHoy) : ''}${k.conPrecio < k.posiciones.length ? ` · ${k.posiciones.length - k.conPrecio} sin precio` : ''}` : 'Falta tu clave de Finnhub en Ajustes'}</span></div><div class="row" style="gap:6px;flex:none;flex-wrap:nowrap;margin-left:auto">${hayKey ? '<button class="btn sm" data-act="claude-contexto">Para Claude</button>' : ''}<button class="btn sm ${hayKey ? 'primary' : ''}" data-act="${hayKey ? 'precios-update' : 'go-config'}">${hayKey ? 'Actualizar' : 'Configurar'}</button></div></div></div>`;
